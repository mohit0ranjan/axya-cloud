...
export const initSchema = async () => {
  const schema = `
        ...
        CREATE TABLE IF NOT EXISTS files (
          ...
          CREATE TABLE IF NOT EXISTS upload_sessions (
            ...
            source_tag TEXT,
            upload_protocol TEXT NOT NULL DEFAULT 'chunk' CHECK (upload_protocol IN ('chunk', 'stream')),
            ...
            status TEXT NOT NULL DEFAULT 'queued' CHECK (status IN ('queued', 'uploading', 'processing', 'paused', 'completed', 'cancelled', 'failed')),
  ...
  const migrations = [
    ...
    `ALTER TABLE files ADD COLUMN IF NOT EXISTS partial_sha256 TEXT`,
    ...
    `CREATE INDEX IF NOT EXISTS idx_files_user_partial_sha ON files (user_id, file_size, partial_sha256) WHERE partial_sha256 IS NOT NULL`,
    ...
    `ALTER TABLE upload_sessions ADD COLUMN IF NOT EXISTS upload_protocol TEXT NOT NULL DEFAULT 'chunk'`,
    `ALTER TABLE upload_sessions ALTER COLUMN upload_protocol SET DEFAULT 'chunk'`,
    `UPDATE upload_sessions SET upload_protocol = 'chunk' WHERE upload_protocol IS NULL OR upload_protocol NOT IN ('chunk', 'stream')`,
    `ALTER TABLE upload_sessions DROP CONSTRAINT IF EXISTS upload_sessions_upload_protocol_check`,
    `ALTER TABLE upload_sessions ADD CONSTRAINT upload_sessions_upload_protocol_check CHECK (upload_protocol IN ('chunk', 'stream'))`,
    ...
    `ALTER TABLE upload_sessions ADD COLUMN IF NOT EXISTS status TEXT NOT NULL DEFAULT 'queued'`,
    `UPDATE upload_sessions SET status = 'queued' WHERE status = 'pending'`,
    `ALTER TABLE upload_sessions ALTER COLUMN status SET DEFAULT 'queued'`,
    `ALTER TABLE upload_sessions DROP CONSTRAINT IF EXISTS upload_sessions_status_check`,
    `ALTER TABLE upload_sessions ADD CONSTRAINT upload_sessions_status_check CHECK (status IN ('queued', 'uploading', 'processing', 'paused', 'completed', 'cancelled', 'failed'))`,
