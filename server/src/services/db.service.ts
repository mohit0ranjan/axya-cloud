...
export const initSchema = async () => {
  const schema = `
        ...
        CREATE TABLE IF NOT EXISTS files (
          ...
          CREATE TABLE IF NOT EXISTS upload_sessions (
            ...
            status TEXT NOT NULL DEFAULT 'queued' CHECK (status IN ('queued', 'uploading', 'processing', 'completed', 'cancelled', 'failed')),
  ...
  const migrations = [
    ...
    `ALTER TABLE upload_sessions ADD CONSTRAINT upload_sessions_status_check CHECK (status IN ('queued', 'uploading', 'processing', 'completed', 'cancelled', 'failed'))`,
