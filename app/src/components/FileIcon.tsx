...
export function getIconConfig(mime: string, themeColors: any) {
    if (!mime || mime === 'inode/directory') return { color: themeColors.primary, bg: '#EEF1FD', Icon: Folder };
    ...
    return { color: themeColors.primary, bg: '#EEF1FD', Icon: FileText };
}
...
export const FileIcon = ({ item = {}, size = 46, token, apiBase, themeColors = { primary: '#4B6EF5' }, style }: any) => {
    ...
    return (
        <View style={[{ width: size, height: size, borderRadius: size * 0.28, justifyContent: 'center', alignItems: 'center', backgroundColor: bg, overflow: 'hidden' }, style]}>
            {canLoadThumb ? (
                <Image
                    source={{
                ...
                <Icon color={color} size={size * 0.5} />
            )}
        </View>
    );
};
